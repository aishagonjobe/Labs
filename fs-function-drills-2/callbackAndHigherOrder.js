...
// CODE HERE
const multiply = (num1, num2, operation) => operation(num1 * num2);
...
const contains = (array, name, callback) => array.includes(name) ? callback(true) : callback(false);
...
const getUserById = (users, id, callback) => callback(users.filter(user => user.id === id)[0]);
